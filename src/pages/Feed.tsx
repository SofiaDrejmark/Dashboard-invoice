...
function Feed() {
  ...
  let amount = invoices.filter(function(invoice){
...
let mapedPrices: number[] = invoices.map(({totalprice}) => totalprice)

let totalSum: number = mapedPrices.reduce(function (a, b) { 
  return a + b;
}, 0);
...
const sum = yearAmount.reduce((ob1: any, ob2: any) => {
  return ob1 + ob2.totalprice
})
console.log(sum)*/
  return (
    <div className="feedDiv">
<div className="amountDiv">Total amount invoices: <span className="amount"> SEK</span></div>

      <ul className="feedUl">
        <Link to="/projectlist"><li className="projectsLi">
          <span>{projectsLength}</span>
          <p>Projects</p>
        </li></Link>
        <Link to="tasklist"><li className="tasksLi">
          <span>{tasksLength}</span>
          <p>Tasks</p>
        </li></Link>
       <Link to="/invoicelist"> <li className="invoicesLi">
